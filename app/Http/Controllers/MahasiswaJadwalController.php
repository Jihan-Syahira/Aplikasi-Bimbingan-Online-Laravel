...
class MahasiswaJadwalController extends Controller
{
    ...
    public function json()
    {
        ...
        $bimbingan = Bimbingan::select('id')->where('kategori', 'KP')->where('id_mahasiswa', $mahasiswa->id)->get()->toArray();
        $data = BimbinganDetail::select('*')
            ...
            ->get();
        foreach($data as $row) {
            $bimbinganDosen = Bimbingan::select('id')->where('kategori', 'KP')->where('id_dosen', $row->cari_bimbingan->id_dosen)->get()->toArray();
